...
def test_rocket_landing_basic():
    ...
    for t in range(int(max_time / dt)):
        ...
        max_gimbal = math.radians(5)  # 降低最大摆角
        ...
        # 限制姿态
        max_theta = math.radians(30)  # 降低最大姿态角
        if theta > max_theta:
            theta = max_theta
        ...
        # 限制角速度
        max_omega = math.radians(20)  # 降低最大角速度
        if omega > max_omega:
            omega = max_omega
...
def main():
    """主函数"""
    # 场景1: 简化着陆
    print("\n场景1: 简化着陆 (高度2000m, 水平距离500m)")
    trajectory1, success1 = test_rocket_landing_basic()
